...
int connects = 100;     //总连接数
int connecteds = 0;//当前时刻完成连接
...
static const struct option long_options[]=
{
  ...
  {"connects",required_argument,NULL,'n'},
  {NULL,0,NULL,0}
};
...
int main(int argc,char* argv[])
{
  ...
  //2.有输入选项则一个一个解析
  while((opt = getopt_long(argc,argv,"frt:p:n:c:?V912",long_options,&options_index)) != EOF)
  {
    switch(opt)
    {
      case 'f':
...
//主线程的作用：读多线程测试到的数据，然后处理
static int bench()
{
  int i = 0;
  //printf("12322141241264%d\n",connects);
  //尝试建立连接一次
